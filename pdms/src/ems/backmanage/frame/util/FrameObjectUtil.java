 ...
 @SuppressWarnings("unchecked") public class FrameObjectUtil {
	...
	public  static boolean valueEqual(Object oldValue, Object newValue) {
		...
		try {
			...
			log.error("比较两个对象字符串值是否相等异常。旧值为："+oldValue+" 新值为："+newValue);
		}
   ...
   */
	...
	/**
	 * 遍历对象的所有属性，将MAP中对应的值赋给对应的属性; 
	 * 1.不管属性是否可访问，是否有get set 方法， 都进行遍历
	 * 2.静态属性也会遍历
	...
	/**
	 * 将一个 Map<String,Object> 对象转化为一个 JavaBean。有get set 方法的属性就可以参与转换,与属性的访问级别无关。静态的属性 即使有get set方法 也不会转换
	 * 
	...
	/**
	 * 将一个 JavaBean 对象转化为一个 Map。有get set 方法的属性就可以参与转换,与属性的访问级别无关。静态的属性 即使有get set方法 也不会转换
	 * 
	 * @param bean  要转化的JavaBean 对象
	 *           
	 * @return 转化出来的 Map<String,Object> 对象
	 */
	 ...
	 */
	  ...
	  public static Object convertValueByFieldType(Class<?> fieldType, Object value) {
			...
			try {
				...
				log.warn("Convert Json Map<String,Object> To Object UnKnowed Type:"
						+ fieldType.toString() + ";value = " + value.toString());
			} catch (Exception e) {
				e.printStackTrace();
				log.debug(log);
			}
