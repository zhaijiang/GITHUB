...
import java.util.regex.Pattern;
 public class FrameStringUtil {
	...
	public static String upper(String s, int len) {
		...
		return s.substring(0, len).toUpperCase() + s.substring(len);//将首字母大写 其他的不变
	}
	
	public static String upper(String s, int start,int end) {
		
		return s.substring(0,start)+s.substring(start,end+1).toUpperCase();//将首字母大写 其他的不变
	}
